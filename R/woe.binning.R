...
if ( length(unique(dfrm[,1]))==2 && is.factor(dfrm[,2]) ) {
	...
	if ( length(which(look.up.table[,2]=='Missing'))==1 && (length(which(look.up.table[,1]=="misc. level neg."))==1 || length(which(look.up.table[,1]=="misc. level pos."))==1) ) {
		if ( (which(look.up.table[,2]=='Missing') == which(look.up.table[,1]=='misc. level pos.')) || (which(look.up.table[,2]=='Missing') == which(look.up.table[,1]=='misc. level pos.')) ) {
...
#' @title Generation of Binning
#'
#' @description
#' \code{woe.binning} generates a supervised binning of numeric variables and factors
#' with respect to a dichotomous target variable.
#'
#' @details
#' \code{woe.binning} generates a supervised binning of numeric variables and factors with
#' respect to a dichotomous target variable. Its parameters provide flexibility in
#' finding a binning that fits specific data characteristics and practical needs.
...
woe.binning <- function(df, target.var, pred.var, min.perc.total, min.perc.class, stop.limit, abbrev.fact.levels, event.class) {
	...
	if ( min.perc.total<0.01 || min.perc.total>0.2 || !is.numeric(min.perc.total) ) { warning("Incorrect parameter specification; accepted min.perc.total parameter range is 0.01-0.2.") }
	if ( missing(min.perc.class)==TRUE ) { min.perc.class=0 }
	if ( min.perc.class<0 || min.perc.class>0.2 || !is.numeric(min.perc.class) ) { warning("Incorrect parameter specification; accepted min.perc.class parameter range is 0-0.2.") }	
	if ( missing(stop.limit)==TRUE ) { stop.limit=0.1 }
	if ( stop.limit<0 || stop.limit>0.5 || !is.numeric(stop.limit) ) { warning("Incorrect parameter specification; accepted stop.limit parameter range is 0-0.05.") }	
	if ( missing(abbrev.fact.levels)==TRUE ) { abbrev.fact.levels=200 }
	if ( abbrev.fact.levels<0 || abbrev.fact.levels>1000 || !is.numeric(stop.limit) ) { warning("Incorrect parameter specification; accepted abbrev.fact.levels parameter range is 0-10000.") }
